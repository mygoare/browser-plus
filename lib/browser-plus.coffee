...
module.exports = BrowserPlus =
  ...
  config:
    ...
    graphTool:
      title: 'GraphTool'
      type: 'string'
      default: 'http://timecore.cn/blockly/apps/max'
    live:
  ...
  activate: (state) ->
    ...
    @subscriptions.add atom.commands.add 'atom-workspace', 'browser-plus:openGraphTool': => @openGraphTool()
  ...
  openGraphTool: (url, opt={})->
    url = atom.config.get('browser-plus.graphTool')
